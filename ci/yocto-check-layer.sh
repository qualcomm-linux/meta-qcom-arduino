...
git -c advice.detachedHead=false -c init.defaultBranch=master clone --quiet --shared $REPO_DIR meta-qcom-arduino

# Yocto Project layer checking tool
CMD="yocto-check-layer"
# Layer to check
CMD="$CMD meta-qcom-arduino"
...
CMD="$CMD --machines $(echo $(find meta-qcom-arduino/conf/machine/ -maxdepth 1 -name *.conf -exec basename {} .conf \; ))"
